...
function enrichWithInvoiceInfo(expense) {
  ...
  expense.invoiceStatus = normalizeInvoiceBusinessStatus(info);
  expense.invoiceRegistrationDate = info.registrationDate || '';
  expense.invoiceExpireDate = info.expireDate || info.disposalDate || '';
  ...
  expense.invoiceNote = buildInvoiceApiNote(info);
...
function normalizeInvoiceBusinessStatus(info) {
  if (!info) return '未確認';
  if (info.status === 'invalid_number') return '番号不正';
  if (info.status === 'not_found' || info.status === '登録なし') return '登録なし';
  if (info.errorMessage && !info.fromCache) return '未確認';
  if (info.disposalDate) return '取消';
  if (info.expireDate) return '失効';

  const status = String(info.status || '').trim();
  if (!status || status === 'api_error' || status === 'not_configured') return '未確認';
  if (/取消/.test(status)) return '取消';
  if (/失効/.test(status)) return '失効';
  if (/公表|有効|登録/.test(status)) return '有効';
  return status;
}

function buildInvoiceApiNote(info) {
  if (!info) return '';

  const notes = [];
  if (info.fromCache) notes.push(info.cacheStale ? 'stale cache 利用' : 'cache hit');
  if (info.errorMessage) notes.push('API error: ' + info.errorMessage);
  if (info.apiErrorMessage) notes.push('API error: ' + info.apiErrorMessage);
  if (!notes.length && info.registeredName) notes.push('API確認済み');
  return notes.join(' / ');
}
